...
function Game() {
  ...
  useEffect(() => {
    try {
      fetch(`http://localhost:8080/events/${eventId}`).then(async (res) => {
  ...
  const onPlayerReady = useCallback(() => {
    socket.emit(
      EVENT_UPDATE_EVENT,
      { eventId, type: "playerReady" },
      { userName }
    );
  }, [eventId, userName]);

  /** Handles player leaves */
  useEffect(() => {
    const beforeUnloadListener = () => {
      if (eventId) {
        socket.emit(
          EVENT_UPDATE_EVENT,
          { eventId, type: "playerLeave" },
          { userName }
        );
      }
    };
  ...
  const boardComponent = useMemo(() => {
    if (gameMetadata.stage === GAME_STAGE.INIT) {
      ...
      gameMetadata.stage === GAME_STAGE.START ||
      gameMetadata.stage === GAME_STAGE.END
    ) {
      return (
        <StartAndEnd
          {...{
